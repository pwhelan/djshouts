...
class FacebookClient(OAuthClient):
	...
	A client for talking to the Facebook API using OAuth
	as the aunthentication model.
	"""
	...
	def get_authorization_url(self):
		permissions = [	'offline_access',	# post automatically
				'read_stream',		# read comments (?)
				'publish_stream',	# post to the news streams
				'manage_pages'		# post on pages
				]
		
		#token = self._get_auth_token()
		return ("https://www.facebook.com/dialog/oauth?client_id=%s"
				"&redirect_uri=%s&scope=%s"
				% ('343474889029815', urlquote(self.callback_url), 
					' '.join(permissions)))
